...
# parser.add_argument('--video_teacher_train_path', type=str, default=r"/home/Students/j_n317/SAKDN/data/debug_train.txt")
# parser.add_argument('--video_teacher_test_path', type=str, default=r"/home/Students/j_n317/SAKDN/data/debug_test.txt")

parser.add_argument('--video_teacher_train_path', type=str, default=r"/home/Students/j_n317/SAKDN/data/UTD_rgb_train_list_subject_specific.txt")
parser.add_argument('--video_teacher_test_path', type=str, default=r"/home/Students/j_n317/SAKDN/data/UTD_rgb_val_list_subject_specific.txt")
parser.add_argument('--student_base',
                        choices=dict(inception_v1bn=backbone.InceptionV1BN,
                                    resnet18=backbone.ResNet18,
                                    resnet50=backbone.ResNet50,
                                    vggnet16=backbone.VggNet16,
                                    TRN=backbone.TRN,
                                    ),
                        default=backbone.TRN,
                        action=LookupChoices)

parser.add_argument('--consensus_type', type=str, default='avg',
                    choices=['avg', 'identity', 'rnn','TRN', 'TRNmultiscale'])
...
opts.num_classes=27
opts.Video_Teacher_base=backbone.TRN_resnet18
...
opts.save_dir= opts.output_dir+'_'.join(map(str, [opts.dataset,'TRN', 
            'seg'+str(opts.num_segments),'epochs'+str(opts.epochs),'batch'+str(opts.batch), 'lr'+str(opts.lr),'dropout'+str(opts.dropout)]))
...
def main():
    ...
    video = opts.Video_Teacher_base(opts.num_classes, opts.num_segments, 'RGB', 
    base_model=opts.arch, consensus_type=opts.consensus_type,  dropout=opts.dropout, partial_bn=True).cuda()
    ...
    policies = video.get_optim_policies()
    train_augmentation = video.get_augmentation()
    normalize = GroupNormalize(input_mean, input_std)
# ========================= video Dataset===============================

    video_teacher_train_loader = torch.utils.data.DataLoader(
        TSNDataSet("", opts.video_teacher_train_path, num_segments=opts.num_segments,
                   new_length=1,
                   modality='RGB',
                   image_tmpl="{:06d}.jpg",
    ...
    video_teacher_test_loader = torch.utils.data.DataLoader(
        TSNDataSet("", opts.video_teacher_test_path, num_segments=opts.num_segments,
                   new_length=1,
                   modality='RGB',
                   image_tmpl="{:06d}.jpg",
                   random_shift=False,
    ...
    if torch.cuda.device_count() > 1:
        video = torch.nn.DataParallel(video, device_ids=[0,1]).cuda()
    logging.info("Number of videos in Training Set: %d" % len(video_teacher_train_loader.dataset))
    logging.info("Number of videos in Testing Set: %d" % len(video_teacher_test_loader.dataset))
...
# =============================Train============================
    def train(t_loader,ep):
        ...
        for (s_videos,s_labels) in t_loader:
            ...
            conv_out_conv2,conv_out_3a,conv_out_3b,conv_out_3c,conv_out_4a,conv_out_4b,conv_out_4c,conv_out_4d,conv_out_4e,conv_out_5a,conv_out_5b,v_semantic,video_output = video(s_videos_combined)
            s_side=len(s_labels)
            ...
            if i % opts.print_freq == 0:
                log_str=('Epoch[{0}]:[{1:03}/{2:03}] '
                        ...
                        ep, i, len(t_loader),cls_loss=loss_cls,top1_recall=top1_recall,top1_prec=top1_prec))
                logging.info(log_str)
            i=i+1
        epoch_time.update(time.time() - end)
        logging.info('[Epoch %d] Time:  %.5f, Loss: %.5f, Training Acc: %.5f\n' %\
            (ep, epoch_time.val, torch.Tensor(loss_all).mean(), 100*train_acc/(len(t_loader.dataset))))

# =============================Evaluation============================
    def eval_video_teacher(net, loader, ep):
        torch.cuda.empty_cache() 
        K=opts.recall
        net.eval()
        correct = 0
        ...
        with torch.no_grad():
            for i,(images, labels) in enumerate(loader, start=1):
                ...
                conv_out_conv2b,conv_out_3a,conv_out_3b,conv_out_3c,conv_out_4a,conv_out_4b,\
    ...
    logging.info('----------- Video Teacher Network performance  --------------')

    best_val_recall, best_val_acc, s_prec, labels_all  = eval_video_teacher(video, video_teacher_test_loader, 0)
    ...
    for epoch in range(1, opts.epochs+1):
        ...
        train(video_teacher_train_loader, epoch)
        val_recall, val_acc, s_prec, labels_all = eval_video_teacher(video, video_teacher_test_loader, epoch)
...
# Save the model
        ...
        if best_val_acc < val_acc:
            ...
            if opts.save_dir is not None:
                if not os.path.isdir(opts.save_dir):
                    os.mkdir(opts.save_dir)
                torch.save(video.state_dict(), "%s/%s"%(opts.save_dir, "best_acc.pth"))
        ...
        if best_combined_acc < val_comb_acc:
            ...
            if opts.save_dir is not None:
                if not os.path.isdir(opts.save_dir):
                    os.mkdir(opts.save_dir)
                torch.save(video.state_dict(), "%s/%s"%(opts.save_dir, "best_combined.pth"))
        ...
        logging.info("Final Eval F-measure: %.4f" % (F_measure*100))
